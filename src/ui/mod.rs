...
fn draw_help(f: &mut Frame, _app: &App, area: Rect) {
    ...
    let text = vec![
        ...
        ratatui::text::Line::from("  j/\u{2193}     - move down"),
        ratatui::text::Line::from("  k/\u{2191}     - move up"),
        ratatui::text::Line::from("  Space   - toggle selection (source/target)"),
        ratatui::text::Line::from("  Enter   - expand/collapse dir"),
        ratatui::text::Line::from("  a       - select/deselect all (source/target)"),
        ratatui::text::Line::from("  s       - sync selected / upload (source panel)"),
        ratatui::text::Line::from("  d       - download selected (target panel)"),
        ratatui::text::Line::from("  Tab     - switch source/target panel"),
        ratatui::text::Line::from("  r       - reload remote tree"),
        ratatui::text::Line::from("  p       - enter password for remote"),
        ratatui::text::Line::from(""),
