...
编译产物位于 `target/release/crabsync`。

### 从 Release 下载

从 [GitHub Releases](https://github.com/anomalyco/crabsync/releases) 下载预编译包：
...
"配对"（pair）将一个本地目录链接到一个远程目标。配对存储在项目根目录下的 `config/rusync/pairs.toml` 中。
...
配对存储在 `config/rusync/pairs.toml` 中：
...
远程目标使用 rsync 远程路径语法（`user@host:/path`）。也支持本地到本地的配对——只需将远程目标设为本地路径即可。
